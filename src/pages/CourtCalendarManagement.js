...
const CourtCalendarManagement = () => {
  ...
  useEffect(() => {
    if (courtId && selectedDate) {
      bookingAPI
        .getBookedSlots(courtId, dayjs(selectedDate).format("YYYY-MM-DD"))
  ...
  const handleBooking = () => {
    ...
    bookingAPI
      .createOwnerBooking(requestBody)
      .then((res) => {
          ...
          >
            ...
            {selectedSlots
              .reduce((total, slot) => {
                ...
                if (hour >= 17) {
                  currentSlotPrice *= 1.1; // Tăng 10% cho giờ cao điểm
                }
...
export default CourtCalendarManagement;
